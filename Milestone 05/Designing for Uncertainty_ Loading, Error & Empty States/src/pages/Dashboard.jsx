...
const Dashboard = () => {
  ...
  const isEmptyMetrics = stats && stats.__isEmpty === true;
  ...
  return (
    <div className="p-8">
      ...
      {isLoading && (
        <>
          ...
          <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 min-h-[300px] flex items-center justify-center">
            <div className="w-full max-w-lg h-48 rounded-lg bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 bg-[length:200%_100%] animate-shimmer" />
          </div>
        </>
      )}
      ...
      {!isLoading && !error && isEmptyMetrics && (
        <EmptyState
      ...
      {!isLoading && !error && stats && !isEmptyMetrics && (
        <>
