...
import { getUserById, updateUserById, isAxiosError } from '../services/api.js'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    getUserById(1)
      ...
      .catch((err) => {
        ...
        const httpMsg = status != null ? `HTTP ${status}` : err.message
        if (String(httpMsg).includes('401')) {
          setError('You need to log in')
        } else {
          setError('Could not load profile: ' + httpMsg)
        }
        setLoading(false)
      })
  }, [])
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true)
    try {
      const updated = await updateUserById(1, form)
      setUser(updated)
