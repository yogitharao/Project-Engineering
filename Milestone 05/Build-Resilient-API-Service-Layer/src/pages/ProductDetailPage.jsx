...
// Notice this is basically copy-pasted from ProductsPage — every dev wired requests differently!

import { useState, useEffect } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { createCart, getProductById, getProductsByCategory, postUser, isAxiosError } from '../services/api.js'
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      ...
      .then((items) => {
        setRelated(items.filter((p) => p.id !== parseInt(id, 10)).slice(0, 3))
        setLoading(false)
      })
      .catch((err) => {
  ...
  const handleAddToCart = async () => {
    try {
      ...
      const msg = isAxiosError(err) && err.response ? 'Cart update failed' : err.message
  ...
  const handleReview = async (e) => {
    ...
    try {
      await postUser({ productId: id, rating: 5 })
          ...
          <div className="border border-gray-200 rounded-xl p-5 flex flex-col gap-3">
            ...
            <button
              ...
              className={`w-full py-2 rounded-lg text-sm font-medium transition-colors ${
                inCart ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-900 text-white hover:bg-gray-700'
              }`}
            >
