...
import { deleteCartById, getCartsByUserId, getProductById } from '../services/api.js'
      ...
      .then(async (carts) => {
        ...
        const productDetails = await Promise.all(
          latest.products.map((item) =>
            getProductById(item.productId).then((p) => ({ ...p, quantity: item.quantity }))
          )
        )
  ...
  if (items.length === 0)
    return (
      <div className="text-center py-24">
        ...
        <a href="/" className="bg-gray-900 text-white text-sm px-5 py-2.5 rounded-lg hover:bg-gray-700 transition-colors">
          Browse Tools
        </a>
      </div>
    )
